---
title: "Calculations and charts - Northern italian provinces (by region) - COVID-19 Epidemic"
...
istat_provinces$zone_code<-substring(istat_provinces$territory_code,1,3)
...
provinces_regions_key <-
  ...
  chosen_regions<-italian_regions
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Match each italian administrative region with its corresponding geographical zone  
istat_regions_temp<-select(istat_regions, region_name, territory_code)
rm(istat_regions_temp)
regions_key<-distinct(select(provinces_regions_key, denominazione_regione, regional_territory_code))
regions_key<-arrange(regions_key, denominazione_regione)
regions_key$zone_code<-substring(regions_key$regional_territory_code,1,3)
regions_zones_key<-inner_join(regions_key, istat_zones, by=c('zone_code'='territory_code'))
revised_col_names <- gsub("territory_name", "zone_name_IT", colnames(regions_zones_key))
colnames(regions_zones_key) <- revised_col_names
revised_col_names <- gsub("denominazione_regione", "region_name", colnames(regions_zones_key))
colnames(regions_zones_key) <- revised_col_names
regions_zones_key$zone_name<-regions_zones_key$zone_name_IT
regions_zones_key$zone_name<-gsub("Centro (I)","Center",regions_zones_key$zone_name, fixed=TRUE)
regions_zones_key$zone_name<-gsub("Sud","South",regions_zones_key$zone_name)
regions_zones_key$zone_name<-gsub("Nord-est","Northeast",regions_zones_key$zone_name)
regions_zones_key$zone_name<-gsub("Nord-ovest","Northwest",regions_zones_key$zone_name)
regions_zones_key$zone_name<-gsub("Isole","Islands",regions_zones_key$zone_name)
regions_zones_key<-select(regions_zones_key, region_name, zone_name, zone_name_IT, zone_code)
provinces_regions_zones_key<-inner_join(provinces_regions_key, regions_zones_key, by=c('denominazione_regione'='region_name'))
```


```{r echo=FALSE, message=FALSE}
# Province names by geographical zone 

northeast_key<-provinces_regions_zones_key[provinces_regions_zones_key$zone_name=="Northeast",]
province_names_northeast<-select(northeast_key,province_name)
region_names_northeast<-arrange(distinct(select(northeast_key,denominazione_regione)),denominazione_regione)

northwest_key<-provinces_regions_zones_key[provinces_regions_zones_key$zone_name=="Northwest",]
province_names_northwest<-select(northwest_key,province_name)
region_names_northwest<-arrange(distinct(select(northwest_key,denominazione_regione)),denominazione_regione)

## North = Northeast + Northwest
north_key<-full_join(northeast_key, northwest_key)
province_names_north<-select(north_key,province_name)
region_names_north<-arrange(distinct(select(north_key,denominazione_regione)),denominazione_regione)
...
```{r echo=FALSE, message=FALSE}
provinces_data<-inner_join(provinces_data, province_names_north, by=c('denominazione_provincia'='province_name'))
...
### `r geographical_zone` province trends observed on `r most_recent_dt`
```{r echo=FALSE, message=FALSE}
...
### `r geographical_zone` provinces per capita (expressed as per thousand people)  ‰ rate.
```{r echo=FALSE, message=FALSE} 
...
### `r geographical_zone` provinces - total cases per density (population per square kilometer).
```{r echo=FALSE, message=FALSE} 
...
### Graphs - Total case progression - `r geographical_zone` provinces by region (for the most recent days observed)
...
histogram_title<-paste(geographical_zone,"provinces histogram - growth in new cases",most_recent_dt)
...
specified_italian_regions<-arrange(distinct(select(provinces_calculations,denominazione_regione)))

for (i in 1:nrow(specified_italian_regions)) {

selected_region <- specified_italian_regions[i,]
