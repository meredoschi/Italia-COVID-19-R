...
national$data<-NULL 
# Revised March 2021
#national<-national[national$dt>='2021-7-1',]
...
#  paste('Intensive care (in red), hospitalized with symptoms and home isolation daily changes',
  paste('Intensive care (in red) daily changes', format(min(national$dt), "%d %b"),
  "-",
  format(max(national$dt), "%d %b %Y")
  )
  
  chart8i <-
  ggplot(data=deltas_nat_labelled) + geom_line(aes(y = intensive_care_delta, x = dt),color = "red") +
...
#  paste('Intensive care (in red), hospitalized with symptoms and home isolation daily changes',
  ...
  chart8i2 <-
  ggplot(data=nat_calc) + geom_line(aes(y = intensive_care_delta/new_positives*1000, x = dt),color = "brown") +
   ggtitle(chart8i_title) + xlab("Date") + ylab("proportion")
...
```{r}
national0<-national %>% arrange(dt) %>% select(tests, hospitalized_with_symptoms,dt)

national1<-head(national0,-1)

earliest_dt<-as.character(min(national0$dt))

colnames(national1)<-c('total_tests_previous_day','total_hospitalized_with_symptoms_previous_day','dt')
national1$dt<-national1$dt+1
national1<-rbind(c(NA,NA,earliest_dt),national1)


national11<-inner_join(national0, national1) %>% tail(-1)
national11$total_tests_previous_day<-as.integer(national11$total_tests_previous_day)
national11$total_hospitalized_with_symptoms_previous_day<-as.integer(national11$total_hospitalized_with_symptoms_previous_day)

national11$tests_on_dt<-national11$tests-(national11$total_tests_previous_day)
national11$hospitalized_with_symptoms_on_dt<-national11$hospitalized_with_symptoms-national11$total_hospitalized_with_symptoms_previous_day
#head(national11)
```
```{r fig.height=5, fig.width=10, echo=FALSE, message=FALSE, warning=FALSE}
...
chart11_title <-
  ...
  chart11 <-
  ggplot(national11,
  aes(y = hospitalized_with_symptoms_on_dt , x =
  dt)) + geom_line(color = "orange") + ggtitle(chart11_title) +
  xlab(paste(format(min(national11$dt), "%d %b"), "-", format(max(national11$dt), "%d %b %Y"))) +
