...
# Draft
num_days_trailing_on_dt <- function(df, specified_dt, positives_remaining_col_name, new_positives_col_name) {

  df<-arrange(df, dt) # Important: it is assumed df has a date column called 'dt'

  df<-df[df$dt<=specified_dt,] # trim to specified dt

  df_specified_dt<-df[df$dt==specified_dt,]

  positives_remaining_col_indx<-grep(paste("^",positives_remaining_col_name,"$",sep=''),colnames(df))
  new_positives_col_indx<-grep(paste("^",new_positives_col_name,"$",sep=''),colnames(df))

  num_positives_remaining<-df_specified_dt[, positives_remaining_col_indx]

  i<-0
  positives_detected_subtotal<-0
  num_days_observed=nrow(df)

  while (i<num_days_observed & positives_detected_subtotal<num_positives_remaining) {

    new_positives_detected_on_dt<-df[num_days_observed-i,new_positives_col_indx]  # on that particular day

#    print(new_positives_detected_on_dt)

    positives_detected_subtotal<-positives_detected_subtotal+new_positives_detected_on_dt
